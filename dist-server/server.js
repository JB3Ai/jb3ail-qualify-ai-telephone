...
const twilioWss = new ws_1.WebSocketServer({ server, path: '/media-stream' });
 ...
 */
async function streamAudioToTwilio(socket, streamSid, audioBuffer) {
    return new Promise((resolve) => {
        let offset = 0;
        function sendNextChunk() {
            if (socket.readyState !== ws_1.WebSocket.OPEN || offset >= audioBuffer.length) {
                resolve();
                return;
            }
            const chunk = audioBuffer.slice(offset, offset + CHUNK_SIZE_BYTES);
            offset += CHUNK_SIZE_BYTES;
            const payload = buffer_1.Buffer.from(chunk).toString('base64');
...
const streamWss = new ws_1.WebSocketServer({ server, path: '/api/twilio/stream' });
streamWss.on('connection', (ws) => {
    ...
    const speechCfg = sdk.SpeechConfig.fromSubscription((process.env.SPEECH_KEY || '').trim(), (process.env.SPEECH_REGION || '').trim().toLowerCase());
    let recognizer = null;
    ...
    // ── 2. TTS → Twilio ────────────────────────────────────────────────────────
    const sendAudioToTwilio = async (textToSpeak, locale) => {
    ...
    const queuePhraseForPlayback = (textToSpeak, locale) => {
    ...
    const onRecognized = async (_s, e) => {
        ...
        try {
            ...
            const aiResponse = await streamNeuralResponse(userSpeech, systemPrompt, ttsBuffer, (phrase) => queuePhraseForPlayback(phrase, callLang));
            if (callSid && activeCalls.has(callSid)) {
    ...
    ws.on('message', async (raw) => {
        try {
            ...
            if (msg.event === 'start') {
                ...
                recognizer = sdk.SpeechRecognizer.FromConfig(speechCfg, autoDetectConfig, sdk.AudioConfig.fromStreamInput(pushStream));
                recognizer.recognized = onRecognized;
                recognizer.startContinuousRecognitionAsync(() => broadcastLog('INFO', `[STREAM_BRIDGE] Recognizer started: ${streamSid} lang=${callLang} mode=${mode}`), (err) => broadcastLog('ERROR', `[STREAM_BRIDGE] Recognizer failed: ${String(err)}`));
            ...
            if (msg.event === 'media') {
                if (isBusy)
                    return; // don't feed STT while Zandi is speaking
                const mulaw = buffer_1.Buffer.from(msg.media.payload, 'base64');
                // Decode 8-bit mu-law → 16-bit linear PCM for the push stream
                const pcm = buffer_1.Buffer.alloc(mulaw.length * 2);
                for (let i = 0; i < mulaw.length; i++)
                    pcm.writeInt16LE(mulawTo16Bit(mulaw[i]), i * 2);
    ...
    ws.sendAudioToTwilio = sendAudioToTwilio;
    ws.on('close', () => {
        recognizer?.stopContinuousRecognitionAsync();
        closePushStream();
        cleanupCallBuffers();
...
const parseGoogleCredentials = (rawValue) => {
    ...
    const normalizePrivateKey = (credentials) => {
    ...
    if (!unwrapped.startsWith('{')) {
        try {
            const decoded = buffer_1.Buffer.from(unwrapped, 'base64').toString('utf8');
    ...
    try {
        ...
        throw new Error(`GOOGLE_KEY_JSON_DATA is not valid JSON. Paste the full service-account JSON object into Render, or provide it as a JSON string/base64 string. Parse error: ${message}`);
    }
};
const getGoogleAuth = async (forceRefresh = false) => {
    ...
    try {
        if (process.env.GOOGLE_KEY_JSON_DATA) {
            ...
            cachedAuth = new googleapis_1.google.auth.GoogleAuth({ keyFile: require('path').join(__dirname, '..', 'google-key.json'), scopes });
...
app.all(['/api/clients/sync-sheets', '/api/clients/sync-sheets/'], async (req, res) => {
    ...
    if (GAS_URL) {
        try {
            ...
            const rows = Array.isArray(gasData) ? gasData : (gasData.rows || gasData.values || []);
...
app.post('/api/twilio/twiml', (req, res) => {
    const mode = req.query.mode || 'OPERATOR';
    const configString = req.query.config;
    let demoConfig = {};
    if (configString) {
        try {
            demoConfig = JSON.parse(decodeURIComponent(configString));
        }
        catch { /* ignore */ }
    }
    const domain = process.env.PUBLIC_DOMAIN || process.env.DOMAIN || (process.env.APP_URL ? new URL(process.env.APP_URL).host : `localhost:${PORT}`);
...
function buildLanguageLogicGate(language) {
    switch (language) {
        case 'nso-ZA':
...
function detectSupportedLanguageFromText(text, fallback = 'en-ZA') {
...
function generateDemoPrompt(demoConfig, _language) {
    let objectiveText = '';
...
`;
function buildSystemPrompt(language, callMeta) {
...
app.post('/api/converse', async (req, res) => {
    ...
    if (!text)
        return res.status(400).json({ error: 'No text provided' });
    try {
        const requestedLanguage = language || 'en-ZA';
